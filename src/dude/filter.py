...
class Filter:
    ...
    def filter_order(self):
        filter = InformationOrderBuilder(self.bot_message[0], self.bot_message[1])
    ...
    def _filter_by_name(self, order):
        ...
        word1 = {
            word1
            for word1 in self.user_message.lower().split()
            if word1 not in stopwords
        }
        word_id = {
            word.lstrip("0")
            for word in order["ID"].split()
            if word.lstrip("0") not in stopwords
        }
        name = {
            word2 for word2 in order["Nome"].lower().split() if word2 not in stopwords
        }
